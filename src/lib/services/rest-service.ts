...
export class RestService<T extends AbstractEntity> {
  ...
  constructor(protected actionsManager: BaseActionsManager,
      ...
      entities.forEach((element: RestEntity): void => {
  ...
  protected request(url: string, action: string[], params: any = null, method: string = null): number {
    ...
    observer.subscribe((response: Object) => {
      ...
      this.ngRedux.dispatch({response: apiResponse, type: this.actionsManager.getHTTPResponseAction(action), request: request});
    }, (err: HttpResponse<any>) => {
      const apiResponse: ApiResponse = new ApiResponse();
      apiResponse.requestData = request;
      apiResponse.status = err.status;
      apiResponse.msg = err.body;
      this.ngRedux.dispatch({type: this.actionsManager.getHTTPErrorAction(action), request: request, response: apiResponse});
    });
